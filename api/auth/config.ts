 ...
 * - GET: Exposes OAuth client IDs (Google/X) to frontend safely.
 * - POST / DELETE: Securely deletes all data owned by the authenticated caller (auth.uid()).
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // GET: Expose OAuth Client IDs safely
  if (req.method === "GET") {
    try {
  ...
  // POST / DELETE: Account & User Data Cascade Deletion for Authenticated User
  if (req.method === "POST" || req.method === "DELETE") {
    try {
      ...
      console.log(`[auth/config API] Initiating full cascade account wipe for user: ${userId}`);
      ...
      if (brandsQueryErr) {
        console.warn("[auth/config API] Notice querying user brands:", brandsQueryErr);
      }
      ...
      if (brandIds.length > 0) {
        for (const table of childTables) {
          try {
            ...
            console.warn(`[auth/config API] Notice deleting from ${table}:`, tErr);
          }
        }
        ...
        try {
          ...
          console.warn("[auth/config API] Storage delete notice:", sErr);
        }
        ...
        try {
          ...
          console.warn("[auth/config API] Brands deletion notice:", bErr);
        }
      }
      ...
      try {
        ...
        console.warn("[auth/config API] Profile deletion notice:", pErr);
      }
      ...
      try {
        if (adminClient.auth && adminClient.auth.admin && typeof adminClient.auth.admin.deleteUser === "function") {
          ...
          if (!delUserErr) {
            ...
            console.warn("[auth/config API] admin.deleteUser notice:", delUserErr);
          }
        }
      } catch (authDelErr) {
        console.warn("[auth/config API] auth deleteUser error:", authDelErr);
      }
      ...
      console.error("[auth/config API] Fatal error:", error);
