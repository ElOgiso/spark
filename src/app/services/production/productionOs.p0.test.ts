...
describe("orchestrate idea → production spec", () => {
  it("builds a valid ProductionSpec and legacy brief from an idea", () => {
    const { spec, brief, validation } = orchestrateIdeaToProductionSpec({
    ...
    assert.ok(shot.compiledPrompt && shot.compiledPrompt.includes("SHOT"));
    assert.ok(shot.provider);
  ...
  it("plans generation tasks with dependencies", () => {
    ...
    assert.ok(tasks.some((t) => t.kind === "video"));
    assert.ok(tasks.some((t) => t.kind === "merge"));
    const video = tasks.find((t) => t.kind === "video");
    assert.ok(video && video.dependsOn.length >= 1);
  });
});
...
describe("QC gates", () => {
  it("preflight passes for orchestrated specs", () => {
    ...
    assert.equal(shotGate.status, "pass");
  });
});
