...
export async function createConversationMessage(
  ...
  return insertRow("executive_conversation_messages", values as any) as any;
}
...
export async function batchCreateConversationMessages(
  ...
  const { data, error } = await (supabase
    .from("executive_conversation_messages") as any)
    .insert(messages as any)
    .select("*");

  if (error) return repositoryError<ConversationMessageRow[]>(error.message);
  return { data: (data ?? []) as any, error: null, source: "supabase" };
}
