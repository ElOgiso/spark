...
export function MobileAnalytics({ onNavigate }: MobileAnalyticsProps = {}) {
  ...
  // 4. Hook Patterns
  const hookPatterns: { hook: string; performance: string }[] = [];
        ...
        {!hasConnectedAccounts ? (
          ...
          <div className="space-y-6 mt-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: "Views", value: formatCount(totalViews), icon: Eye, change: "—" },
                { label: "Engagement", value: avgEngagement, icon: Heart, change: "—" },
                { label: "Comments", value: formatCount(totalFollowers), icon: MessageCircle, change: "—" },
              ].map((m) => {
                const Icon = m.icon;
                return (
                  <button
                  ...
                  >
                    ...
                    <p className="text-[10px] text-muted-foreground font-medium mt-1 leading-none">{m.change}</p>
                  </button>
                );
              })}
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="space-y-3">
                {topPerformers.length === 0 && <p className="text-xs text-muted-foreground">{ANALYTICS_EMPTY_COPY}</p>}
                {topPerformers.map((c) => (
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="space-y-3">
                {failures.length === 0 && <p className="text-xs text-muted-foreground">{ANALYTICS_EMPTY_COPY}</p>}
                {failures.map((f) => (
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="space-y-2">
                {hookPatterns.length === 0 && <p className="text-xs text-muted-foreground">{ANALYTICS_EMPTY_COPY}</p>}
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="space-y-3">
                {finalProductions.map((p, i) => (
                  ...
                  >
                    <span className="text-sm font-medium flex-shrink-0 text-muted-foreground">{p.score == null ? "—" : `${p.score}%`}</span>
