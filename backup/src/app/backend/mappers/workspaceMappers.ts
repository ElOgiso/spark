...
export function viralSparkRowToDomain(row: ViralSparkRow): ViralSpark {
  ...
  let riskVal: "Low" | "Medium" | "High" = "Low";
  const rawRisk = row.risk ?? String(evidence.riskLevel ?? "Low");
  if (rawRisk === "High" || rawRisk === "Medium") {
    riskVal = rawRisk;
  }
  ...
  return {
    id: row.id,
    title: row.title,
    platformFit: String(evidence.platforms ?? "YouTube + TikTok"),
    ...
    audienceEmotion: String(evidence.audienceEmotion ?? "Curiosity & Aspiration"),
    expectedRetention: String(evidence.expectedRetention ?? "High retention due to rapid visual hook"),
    difficulty: String(evidence.difficulty ?? "Medium"),
    riskLevel: riskVal,
