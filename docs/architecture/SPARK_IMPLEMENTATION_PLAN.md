...
Implementation target: `main`. Phase 21 validation is **FAIL** as a live certification. Phase 21R attached credits and durable checkpoints in code and did not live-certify. Phase 22 is the release-candidate gate: local architecture is ready where tested; live provider certification and deployment stay **DEFERRED**. See `SPARK_PHASE_22_PRODUCTION_READINESS_RECORD.md`. Do not call that deferral a software failure, and do not call the product fully production-certified.
...
| 8 Credits | Existing profile balance/ledger; CreditService and reservation migration | Implemented `CreditService`, secured financial RPCs, and test matrix A–O. `credit_reservations` is present on live project `jaqzjhabmtvqtvinoafq` (anon SELECT returns 42501, not a missing-table 404). Do not describe that table as still awaiting its first apply. |
...
| 20 UI integration | Existing SPARK / MY SPARK / VIRAL SPARKS / REVIEW / CALENDAR / ANALYTICS / MORE surfaces extended. Canonical mode copy, automation labels, credit balance, billing history, measured analytics only, master-alone is not Ready for Review, unknown submission blocks retry | Implemented on main (`3d29346`, `9daf9ea`). See `SPARK_PHASE_20_UI_INTEGRATION_RECORD.md`. Paid generation confirmation still does not call CostEngine from JSX. |
| 21 Production validation | Local FFmpeg mastering restored and verified. Full unit suite 1427/1427. Live Supabase project answers auth health. Browser reached the auth shell at 1440 and 390. | **FAIL.** Real Narrator/Hybrid/Cinematic provider runs were not executed (no server provider keys or service role in this environment; connected Vercel team does not host spark). Live generate still does not attach CreditService. In-memory idempotency does not survive process restart. See `SPARK_PHASE_21_FULL_PRODUCTION_VALIDATION_RECORD.md`. |
| 21R Blocker remediation | Live `generateAssetsForProduction` now passes `CreditService` and `requireCredits`. Execution checkpoints flush before provider submit. `requestProductionVideoClip` production callers outside `videoI2vAdapter` are removed. `conversation_sessions` migration adds owner RLS and text ids. | **FAIL.** Not applied live. No test identity, no paid Narrator/Hybrid/Cinematic master, no deployed Spark runtime. Asset-bible and studio image calls can still spend through ModelRouter. See `SPARK_PHASE_21R_READINESS_BLOCKER_REMEDIATION_RECORD.md`. |
| 22 Release candidate | One production spine, studio-sheet credit boundary, durability and economics tests, security/migration/config manifests, runbooks, release gates | **RELEASE CANDIDATE READY — LIVE ACCEPTANCE DEFERRED.** Asset-bible sheets require `studioBilling` and are not called by `generateAssets`. Live Narrator/Hybrid/Cinematic, deployed FFmpeg, and applying `20260926013000_conversation_sessions_secure.sql` are launch-gate items, not code failures under the no-credit constraint. See `SPARK_PHASE_22_PRODUCTION_READINESS_RECORD.md` and `SPARK_RELEASE_STATUS.md`. Phase 23 was not started. |
...
4. **E/G — compiler and QC (compiler locally verified, genuine-media QC deferred):** `GenerationExecutionEngine.executeTask` calls `ProviderPayloadCompiler.compile` before adapter submit. Live generate passes `CreditService`. Asset-bible prop/location/wardrobe sheets are a separate studio operation: they reserve through `CreditService` only when `studioBilling` is passed, and production generate does not call them. Genuine-media QC remains a live acceptance item.
...
9. **Higgsfield role boundary (after the wiring pass):** the existing Seedance translator now uses the official image-to-video and reference-to-video bodies. An I2V model keeps `image_url` / `end_image_url` and records when identity references cannot ride that wire. An explicit R2V model compiles `image_urls` plus direct `video_urls` / `audio_urls`. Watch-page source video is not a provider input. Missing a start frame does not become text-to-video and does not silently swap models. The live adapter passes reference mode only for that compiled operation. No second compiler, studio, or catalog was added.
10. **Spark jobs, not vendor skills (after the role boundary):** the nine upstream playbooks are audited in `skills/sparkSkills.ts`. Spark keeps six jobs — generate, character identity, narrated explainer, product still, brand lock, and cover frame — and rejects marketplace cards, websites, and games. The job is recorded on `ProductionSpec.meta.sparkSkill` and does not change production mode, route a model, or spend. The existing filmmaking-skill runtime teaches that job. A product still maps onto the existing craft planner. No Higgsfield CLI, Soul training, explainer assembler, or second generate path was added. Phase 23 was not started.
